// src/components/TPAFCircularVisualization.tsx
import React, { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import { Settings, Info, Download, RefreshCw, Search } from 'lucide-react';
import { useTPAFData } from '../hooks/useTPAFData';
import type { Policy, Dimension, Phase } from '../services/tpafDataService';
import Breadcrumbs from './Breadcrumbs';
import type { BreadcrumbItem } from './Breadcrumbs';
import coverImage from '/Users/elona/Desktop/UNESCO/G20 work/tpaf-visualizer/src/assets/only-cover.png';
import g20Logo from '/Users/elona/Desktop/UNESCO/G20 work/tpaf-visualizer/src/assets/g20-logo.png';
            ...
            <div className="bg-white rounded-2xl p-3 shadow-md min-h-[600px] border border-gray-100 relative">
              <svg ref={svgRef} viewBox="0 0 800 500" className="w-full h-full" style={{ overflow: 'visible' }}>
                ...
                {policySegments.map((segment, index) => (
                  <path
                    key={`segment-${segment.policy.id}-${index}`}
                    d={`
                    ...
                    stroke="#ffffff"
                    strokeWidth="1"
                    onMouseEnter={() => handleMouseEnter(segment.policy)}
                    onMouseLeave={handleMouseLeave}
                    onClick={() => setSelectedPolicy(segment.policy)}
            ...
            {/* <div className="bg-white rounded-xl p-4 shadow-md">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-black font-semibold">{visiblePolicies.length}/{data.policies.length}</span>
                </div>
                {selectedPolicy && (
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-600">Connections</span>
                    <span className="text-black font-semibold">{selectedPolicy.connections?.length || 0}</span>
                  </div>
                )}
                {showConnections && (
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-600">Active Links</span>
                    <span className="text-black font-semibold">{getVisibleConnections().length}</span>
                  </div>
                )}
              </div>
            </div> */}
              ...
              <div className="space-y-2">
                <button 
                  className={`w-full text-left px-3 py-2 rounded text-xs transition-colors ${
                    showConnections ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                  onClick={() => setShowConnections(!showConnections)}
                >
                  {showConnections ? 'Hide' : 'Show'} Connections
                ...
                <button 
                  className="w-full text-left px-3 py-2 rounded text-xs bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
                  onClick={() => {
                    reloadData();
                    resetAllSelections();
                  }}
                ...
                <button 
                  className="w-full text-left px-3 py-2 rounded text-xs bg-blue-100 text-blue-800 hover:bg-blue-200 transition-colors"
                >

...
const TPAFHexagonalVisualization: React.FC = () => {
  ...
  const hexRadius = 60; // Larger hexagons for better UX
  ...
  const hexagonalCells = useMemo(() => {
    ...
    if (navigationLevel === 'overview') {
      ...
      data.dimensions.slice(0, 5).forEach((dimension, index) => {
        const pos = axialToPixel(dimensionPositions[index].q, dimensionPositions[index].r);
        cells.push({
          id: dimension.id,
          dimension,
          x: pos.x,
      ...
      relevantPhases.slice(0, 4).forEach((phase, index) => {
        const pos = axialToPixel(phasePositions[index].q, phasePositions[index].r);
      ...
      // Policies around center in rings
      ...
      const ring2Positions = [
        ...
        { q: -1, r: 2 }, { q: 0, r: 2 }, { q: 1, r: 1 }, { q: 2, r: 0 }
      ];

      const allPositions = [...ring1Positions, ...ring2Positions];

      phasePolicies.slice(0, allPositions.length).forEach((policy, index) => {
        const pos = axialToPixel(allPositions[index].q, allPositions[index].r);
              ...
              >
                ...
                {hexagonalCells.map((cell) => {
                  ...
                  const isCenter = cell.type === 'center' || (
                    (navigationLevel === 'dimension' && cell.id === selectedDimensionId) ||
                    ...
                    <g key={cell.id}>
                      ...
                      <path
                        ...
                        className={`cursor-pointer transition-all duration-200 ${
                          cell.type !== 'center' ? 'hover:brightness-110' : ''
                        }`}
                ...
                {navigationLevel === 'overview' && (
                  <g transform="translate(50, 50)">
                    ...
                    <text x="20" y="30" className="font-bold fill-gray-800" fontSize="16">Navigate the Framework</text>
                    <text x="20" y="55" className="fill-gray-600" fontSize="12">Click on any dimension to explore</text>
                    <text x="20" y="75" className="fill-gray-600" fontSize="12">its phases and policies</text>
                    <text x="20" y="100" className="fill-blue-600" fontSize="11">• Blue hexagons = Clickable elements</text>
                  </g>
                )}
